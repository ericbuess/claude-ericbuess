...
SESSION_CACHE="$REPO_ROOT/session-data/cache/session-status.json"
...
if [ -n "$MODEL_DISPLAY" ]; then
    echo -e "${MODEL_DISPLAY} ${USER_HOST_DIR}${PROJECT_TYPE}${MEMORY_STATUS} | ${SESSION_INFO}${TIME_INFO}${TOKEN_INFO}${VERSE_INFO}${GIT_INFO}${PERM_MODE}"
else
    echo -e "${USER_HOST_DIR}${PROJECT_TYPE}${MEMORY_STATUS} | ${SESSION_INFO}${TIME_INFO}${TOKEN_INFO}${VERSE_INFO}${GIT_INFO}${PERM_MODE}"
fi
...
VERSE_INFO=$(printf ' \033[01;36m%s\033[00m' "$VERSE")

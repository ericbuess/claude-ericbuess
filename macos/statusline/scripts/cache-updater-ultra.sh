...
# Add PATH for cron execution (macOS)
export PATH="/Users/ericbuess/.nvm/versions/node/v22.17.1/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:$PATH"
...
if command -v bunx &> /dev/null; then
    RUNNER="bunx"
elif [ -x "/Users/ericbuess/.nvm/versions/node/v22.17.1/bin/npx" ]; then
    RUNNER="/Users/ericbuess/.nvm/versions/node/v22.17.1/bin/npx"
...
# Note: macOS bash doesn't support associative arrays, using simple variables
INPUT_PRICE_OPUS=15.0
OUTPUT_PRICE_OPUS=75.0
INPUT_PRICE_SONNET=3.0
OUTPUT_PRICE_SONNET=15.0
INPUT_PRICE_HAIKU=0.25
OUTPUT_PRICE_HAIKU=1.25
...
if command -v jq &> /dev/null; then
    ...
    if [ -n "$ACTIVE_SESSION" ]; then
        ...
        if [ -n "$START_TIME" ] && [ -n "$END_TIME" ]; then
            ...
            END_EPOCH=$(date -j -f "%Y-%m-%dT%H:%M:%S" "${END_TIME%%.*}" +%s 2>/dev/null || echo 0)
            NOW_EPOCH=$(date +%s)
    ...
    if [ -f ".gitignore" ]; then
        ...
        MOST_RECENT_CODE_TIME=$(git ls-files -z '*.py' '*.js' '*.ts' '*.jsx' '*.tsx' '*.sh' '*.go' '*.rs' '*.java' '*.cpp' '*.c' 2>/dev/null | xargs -0 stat -f %m 2>/dev/null | sort -n | tail -1)
    else
        # Fallback to find
        # macOS find command - use stat instead of -printf
        MOST_RECENT_CODE_TIME=$(find . -type f \( -name "*.py" -o -name "*.js" -o -name "*.ts" -o -name "*.jsx" -o -name "*.tsx" -o -name "*.sh" -o -name "*.go" -o -name "*.rs" \) -not -path "*/node_modules/*" -not -path "*/.git/*" -exec stat -f %m {} \; 2>/dev/null | sort -n | tail -1)
    fi
    ...
    for CLAUDE_MD in $CLAUDE_MD_FILES; do
        if [ -f "$CLAUDE_MD" ]; then
            ...
            # macOS stat command
            MD_MTIME=$(stat -f %m "$CLAUDE_MD" 2>/dev/null || echo 0)
            ...
            if [ -f "$CLAUDE_MD" ]; then
                ...
                for REF_FILE in $REFERENCED_FILES; do
                    ...
                    if [ -f "$REF_FILE" ]; then
                        # macOS stat command
                        REF_MTIME=$(stat -f %m "$REF_FILE" 2>/dev/null || echo 0)
